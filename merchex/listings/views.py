...
def band_added(request):
    return render(request, 'listings/band_added.html')
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            send_mail(subject=f'Message from {form.cleaned_data["name"] or "anonymous"} via MerchEx contact form',
